  ...
  try {
    ...
    for (let day = 1; day <= dim; day++) {
      ...
      try {
        ...
        // 알바: 시급 × 시간
        (data.alba || []).forEach(it => { total += (it.amount||Math.round((it.wage||0)*(it.hours||0))); });
        // 배달/대리: 건수 × 단가
...
function renderBudgetPage() {
  ...
  <div style="background:${wBgs[wLevel]};border:2px solid ${wBorders[wLevel]};border-radius:12px;
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;">
      ...
      <div style="flex:1;min-width:120px;background:var(--surface);border-radius:8px;padding:8px 10px;">
        ...
        <div style="font-size:14px;font-weight:700;color:${calc.remaining>0?'var(--green)':'var(--red)'};">
          ${calc.remaining>0?calc.remaining.toLocaleString()+'원':'초과!'}
        </div>
      </div>
    </div>
    ${wLevel>=1?`
    <div style="margin-bottom:8px;padding:6px 10px;border-radius:6px;background:rgba(0,0,0,.05);font-size:11px;color:var(--text2);">
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
    ...
    <div style="font-size:12px;color:var(--text2);margin-bottom:8px;">현재 소비 패턴에서 <b>30% 절약</b> 시:</div>
    <div style="display:flex;gap:8px;">
      <div style="flex:1;background:rgba(61,214,140,.08);border:1px solid rgba(61,214,140,.2);
        ...
        <div style="font-size:20px;font-weight:900;color:var(--green);">+${calc.extraDays||0}일</div>
      </div>
    ...
    <div style="margin-top:8px;padding:6px 10px;background:rgba(255,209,102,.08);border-radius:6px;font-size:11px;color:var(--text3);">
      💡 하루 ${Math.round(calc.dailyAvg*0.3).toLocaleString()}원만 아껴도 ${calc.extraDays||0}일 더 유지돼요
    </div>
  </div>`:''}
